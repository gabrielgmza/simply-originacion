...
import { CheckCircle2, ChevronRight, Loader2, Save, Percent, MessageSquare, Key, CreditCard, Lock, Building2, Rocket, AlertTriangle } from "lucide-react";
...
// Fuera del componente para evitar perdida de foco
...
const Toggle = ({ label, desc, value, onChange, color }: any) => (
  <div className="flex items-start justify-between gap-3">
    ...
    <button onClick={() => onChange(!value)} type="button"
      ...
      style={{ backgroundColor: value ? (color || "#FF5E14") : "#374151" }}>
...
function SetupWizard() {
  ...
  const [tasas, setTasas] = useState({ tasaInteresBase: "", gastosOtorgamiento: "", seguroVidaPorc: "", tasaMoratoria: "0.12", tasaPunitoria: "0.12" });
  const [ws, setWs]       = useState({ activo: false, accessToken: "", phoneNumberId: "", wabaId: "" });
  const [cuad, setCuad]   = useState({ usuario: "", password: "" });
  const [p360, setP360]   = useState({ apiKey: "", maxReintentos: "2", diasReintento: "5" });
  const [seguridad, setSeguridad] = useState({ requierePin: false, pin: "", validarCbu: true, validarFirma: true, whatsappAuto: true, exportarExcel: true, registrarTransferencia: true });
  ...
  const crearSucursal = async () => {
    ...
    try {
      await fetch("/api/sucursales", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ entidadId, nombre: sucursalNombre, esMatriz: true }) });
  ...
  const avanzar = (paso: number) => {
    ...
    if (paso === 1 && !tasas.tasaInteresBase) errs.tasaInteresBase = "Requerida";
    if (paso === 5 && seguridad.requierePin && seguridad.pin.length < 4) errs.pin = "Minimo 4 digitos";
    setErrores(errs);
  ...
  if (completado) return (
    <div className="min-h-screen bg-[#050505] flex items-center justify-center p-4">
      <div className="text-center space-y-5 max-w-sm">
        <div className="w-20 h-20 rounded-3xl flex items-center justify-center mx-auto" style={{ backgroundColor: C }}><Rocket size={36} className="text-white"/></div>
        <h1 className="text-2xl font-black text-white">Setup completado!</h1>
        <p className="text-gray-400 text-sm">Tu plataforma esta lista.</p>
        <button onClick={() => router.push("/dashboard")} className="w-full py-3.5 rounded-2xl font-black text-white" style={{ backgroundColor: C }}>Ir al dashboard</button>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#050505] flex items-center justify-center p-4">
      <div className="w-full max-w-lg space-y-5">
        <div className="text-center">
          <p className="text-2xl font-black text-white">Configuracion inicial</p>
          ...
          {PASOS.map((p, i) => {
            ...
            return (
              <div key={p.id} className="flex items-center gap-1">
                <div className={`w-8 h-8 rounded-xl flex items-center justify-center transition-all ${done ? "bg-green-600" : activo ? "" : "bg-gray-900 border border-gray-800"}`} style={activo ? { backgroundColor: C } : {}}>
        ...
        <div className="bg-[#0A0A0A] border border-gray-800 rounded-2xl p-6 space-y-4">
          ...
          {pasoActual === 1 && (<>
            <p className="text-xs text-gray-500 uppercase font-bold tracking-widest">Tasas y parametros financieros</p>
          ...
          {pasoActual === 2 && (<>
            ...
            <Toggle label="Activar WhatsApp" desc="Notificaciones automaticas a clientes" value={ws.activo} onChange={(v:boolean) => setWs(p=>({...p,activo:v}))} color={C}/>
            {ws.activo && (
            ...
            {!ws.activo && <p className="text-xs text-gray-600">Podas configurarlo despues en Configuracion WhatsApp.</p>}
          </>)}
          ...
          {pasoActual === 3 && (<>
            ...
            {modulos.cuad ? (
              <div className="space-y-3">
                <div className="flex items-center gap-2 p-3 rounded-xl bg-green-900/10 border border-green-900/30 text-xs text-green-400"><CheckCircle2 size={12}/> Modulo CUAD habilitado</div>
                <Input label="Usuario CUAD" value={cuad.usuario} onChange={(v:string)=>setCuad(p=>({...p,usuario:v}))} placeholder="usuario_gobierno"/>
                <Input label="Contrasena CUAD" type="password" value={cuad.password} onChange={(v:string)=>setCuad(p=>({...p,password:v}))} placeholder="••••••••"/>
              </div>
            ) : (
              <div className="flex items-center gap-2 p-4 rounded-xl bg-gray-900/40 border border-gray-800 text-sm text-gray-500">
                <AlertTriangle size={14} className="text-yellow-500"/> Modulo CUAD no habilitado. Contacta a Paysur para activarlo.
              </div>
            )}
          </>)}
            ...
            {modulos.pagos360 ? (
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <Input label="Max reintentos" value={p360.maxReintentos} onChange={(v:string)=>setP360(p=>({...p,maxReintentos:v}))} placeholder="2"/>
                  <Input label="Dias entre reintentos" value={p360.diasReintento} onChange={(v:string)=>setP360(p=>({...p,diasReintento:v}))} placeholder="5"/>
                </div>
              </div>
            ) : (
              <div className="flex items-center gap-2 p-4 rounded-xl bg-gray-900/40 border border-gray-800 text-sm text-gray-500">
                <AlertTriangle size={14} className="text-yellow-500"/> Modulo Pagos 360 no habilitado.
              </div>
            )}
          </>)}

          {pasoActual === 5 && (<>
            <p className="text-xs text-gray-500 uppercase font-bold tracking-widest">Liquidacion y seguridad</p>
            <div className="space-y-4">
              <Toggle label="PIN para liquidar" desc="Requiere codigo antes de ejecutar lotes" value={seguridad.requierePin} onChange={(v:boolean)=>setSeguridad(p=>({...p,requierePin:v}))} color={C}/>
              {seguridad.requierePin && <Input label="PIN (4-8 digitos)" type="password" value={seguridad.pin} onChange={(v:string)=>setSeguridad(p=>({...p,pin:v}))} error={errores.pin}/>}
              <div className="h-px bg-gray-900"/>
              <Toggle label="WhatsApp automatico al liquidar" value={seguridad.whatsappAuto} onChange={(v:boolean)=>setSeguridad(p=>({...p,whatsappAuto:v}))} color={C}/>
              <Toggle label="Validar CBU antes de liquidar" value={seguridad.validarCbu} onChange={(v:boolean)=>setSeguridad(p=>({...p,validarCbu:v}))} color={C}/>
              <Toggle label="Validar firma antes de liquidar" value={seguridad.validarFirma} onChange={(v:boolean)=>setSeguridad(p=>({...p,validarFirma:v}))} color={C}/>
              <Toggle label="Registrar Nro de transferencia" value={seguridad.registrarTransferencia} onChange={(v:boolean)=>setSeguridad(p=>({...p,registrarTransferencia:v}))} color={C}/>
              <Toggle label="Exportar Excel del lote" value={seguridad.exportarExcel} onChange={(v:boolean)=>setSeguridad(p=>({...p,exportarExcel:v}))} color={C}/>
            </div>
          </>)}
          ...
          {pasoActual === 6 && (<>
            ...
            {!sucursalCreada ? (
              <div className="space-y-3">
                <Input label="Nombre de la sucursal" value={sucursalNombre} onChange={setSucursalNombre} placeholder="Casa Central / Mendoza"/>
                <button onClick={crearSucursal} disabled={!sucursalNombre.trim() || creandoSucursal}
                  ...
                  {creandoSucursal ? <Loader2 size={13} className="animate-spin"/> : <Building2 size={13}/>} Crear sucursal
                </button>
                <p className="text-[10px] text-gray-600">Podas agregar mas sucursales despues.</p>
              </div>
            ) : (
              <div className="flex items-center gap-2 p-3 rounded-xl bg-green-900/10 border border-green-900/30 text-sm text-green-400">
                <CheckCircle2 size={14}/> Sucursal "{sucursalNombre}" creada
              </div>
            )}
          </>)}

          <button onClick={() => avanzar(pasoActual)} disabled={guardando}
            className="w-full py-3.5 rounded-xl font-black text-white flex items-center justify-center gap-2 mt-2 disabled:opacity-60" style={{ backgroundColor: C }}>
            {guardando ? <Loader2 size={15} className="animate-spin"/> : pasoActual === PASOS.length ? <><Rocket size={15}/> Finalizar setup</> : <><Save size={15}/> Guardar y continuar <ChevronRight size={14}/></>}
          </button>
          {pasoActual > 1 && pasoActual < PASOS.length && (
            <button onClick={() => setPasoActual(p => p + 1)} className="w-full py-2 text-xs text-gray-600 hover:text-gray-400">Omitir este paso</button>
          )}
        </div>
      </div>
    </div>
