...
export async function POST(request: Request) {
  try {
    const { operacionId, entidadId, usuarioGenerador } = await request.json();
    ...
    const qPlantillas = query(collection(db, "plantillas"), where("entidadId", "==", entidadId), where("mapeoConfigurado", "==", true));
    ...
    if (coordenadas.aclaracion) firstPage.drawText(operacion.cliente.nombre, { x: calcX(coordenadas.aclaracion.x), y: calcY(coordenadas.aclaracion.y), size: 11, color: colorTexto });
    if (coordenadas.dni) firstPage.drawText(`DNI: ${operacion.cliente.dni}`, { x: calcX(coordenadas.dni.x), y: calcY(coordenadas.dni.y), size: 11, color: colorTexto });
    ...
    if (coordenadas.monto) {
      ...
      firstPage.drawText(montoFormateado, { x: calcX(coordenadas.monto.x), y: calcY(coordenadas.monto.y), size: 11, color: colorTexto });
    }
    
    if (coordenadas.fecha) {
      const fechaActual = new Date().toLocaleDateString('es-AR');
      firstPage.drawText(fechaActual, { x: calcX(coordenadas.fecha.x), y: calcY(coordenadas.fecha.y), size: 11, color: colorTexto });
    }

    if (coordenadas.firmaTitular && firmaImage) {
      const imgWidth = 120; const imgHeight = 60;
      firstPage.drawImage(firmaImage, { x: calcX(coordenadas.firmaTitular.x) - (imgWidth / 2), y: calcY(coordenadas.firmaTitular.y) - (imgHeight / 2), width: imgWidth, height: imgHeight });
    }
    ...
    await updateDoc(opRef, { "legajo.contratoFinalPdf": contratoFinalUrl, fechaActualizacion: serverTimestamp() });

    // Registro en el LOG de Auditoría
    await addDoc(collection(db, "logs_operaciones"), {
      operacionId: operacionId,
      entidadId: entidadId,
      usuario: "SISTEMA (Motor PDF)",
      accion: "COMPILACION_CONTRATO",
      detalles: `Se fusionó la firma del cliente con la plantilla ${plantillaElegida.nombre}. Accion solicitada por: ${usuarioGenerador || 'Desconocido'}.`,
