...
export default function OriginacionPage() {
  ...
  // Generador del texto para enviar por WhatsApp
  const generarLinkWhatsapp = () => {
    if (!cuadData || !operacionId) return '#';
    const cuotaMensual = calcularCuota().toLocaleString(undefined, {maximumFractionDigits: 0});
    const capital = parseFloat(monto).toLocaleString();
    
    // Aquí pondremos el enlace real a la webapp del cliente en el futuro
    const linkFirma = `https://simply-originacion.vercel.app/firma/${operacionId}`; 
    
    const texto = `Hola *${cuadData.nombre}* 👋,\n\nTe escribo de PaySur. Tu crédito por *$${capital}* en ${cuotas} cuotas fijas de *$${cuotaMensual}* está pre-aprobado.\n\nPara avanzar con el depósito, por favor ingresa a este link seguro para validar tu identidad y firmar la solicitud:\n👉 ${linkFirma}\n\nCualquier duda, estoy a disposición.`;
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto text-black bg-gray-50 min-h-screen">
      ...
      <div className="bg-white p-8 shadow-lg rounded-xl border border-gray-100 min-h-[400px]">
        ...
        {step === 1 && (
          <form onSubmit={handleConsultarDni} className="space-y-6 text-center max-w-sm mx-auto py-10 animate-fade-in-up">
            ...
            <p className="text-sm text-gray-500">Ingresa el DNI para consultar en el sistema CUAD del Gobierno.</p>
            ...
            <button type="submit" disabled={loading || dni.length < 7} className="w-full bg-blue-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-blue-700 disabled:bg-blue-300 transition-colors flex justify-center items-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="bg-gray-800 text-white p-6 rounded-xl flex flex-col justify-center space-y-4 shadow-inner">
                <div className="text-center">
                  ...
                  <p className="text-4xl font-bold text-blue-400">${calcularCuota().toLocaleString(undefined, {maximumFractionDigits: 0})}</p>
                </div>
                
                {calcularCuota() > cuadData.margenAfectable ? (
                  <div className="bg-red-500/20 text-red-300 p-2 rounded text-sm text-center border border-red-500/50">⚠️ Cuota supera margen.</div>
                ) : (
                ...
                <button 
                  disabled={!monto || calcularCuota() > cuadData.margenAfectable || loading}
        ...
        {step === 3 && operacionId && (
          <div className="space-y-6 animate-fade-in-up text-center py-4">
            ...
            <p className="text-gray-500 text-sm">Se ha guardado en la base de datos con éxito.</p>
            
            <div className="bg-gray-50 rounded-lg p-5 text-left max-w-sm mx-auto border border-gray-200">
              ...
              <div className="border-t border-gray-200 pt-2 mt-2">
                <p className="flex justify-between text-sm"><span className="text-gray-600 font-bold">Cuota:</span> <span className="font-bold text-blue-600">{cuotas} cuotas de ${calcularCuota().toLocaleString(undefined, {maximumFractionDigits: 0})}</span></p>
              </div>
            </div>
