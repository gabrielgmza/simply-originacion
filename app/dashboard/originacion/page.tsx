        ...
        {step === 1 && (
          <form onSubmit={handleConsultar} className="max-w-xl mx-auto py-24 px-10 space-y-12 animate-in zoom-in-95">
             ...
             <button type="submit" disabled={loading || dni.length < 7} className="w-full bg-indigo-600 text-white font-black py-8 rounded-[2.5rem] text-xl shadow-2xl shadow-indigo-500/20 hover:scale-[1.02] active:scale-95 transition-all uppercase italic tracking-widest leading-none">
                {loading ? 'Calculando...' : 'Iniciar Evaluación'}
             </button>
          </form>
        )}
        {step === 2 && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-0 animate-in slide-in-from-bottom-10">
             <div className="lg:col-span-7 p-12 space-y-12 border-r dark:border-white/5 font-black">
                <div className="bg-slate-50/50 dark:bg-white/[0.02] p-8 rounded-[2.5rem] border border-slate-200 dark:border-white/5 shadow-inner flex items-center justify-between">
                   <div className="flex items-center space-x-6">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-indigo-500 uppercase tracking-widest mt-2 leading-none italic font-black">BCRA SIT: {bcraData?.situacion}</p>
                      </div>
                   </div>
                   <div className="text-right">
                      <p className="text-[10px] text-slate-400 uppercase tracking-widest leading-none mb-1 italic font-black">Margen</p>
                      <p className="text-xl text-slate-900 dark:text-white leading-none font-black">${cuadData?.margenAfectable?.toLocaleString()}</p>
                   </div>
                </div>
                <div className="grid grid-cols-2 gap-8 font-black italic">
                   <div className="space-y-3">
                      <label className="text-[10px] text-slate-400 uppercase tracking-widest ml-6 leading-none">Monto ($)</label>
                      <input type="number" value={monto} onChange={e => setMonto(e.target.value)} className="w-full bg-slate-50 dark:bg-white/[0.02] border border-slate-200 dark:border-white/10 rounded-[1.5rem] px-6 py-5 text-3xl text-slate-950 dark:text-white focus:ring-4 focus:ring-indigo-500/10 outline-none shadow-sm" />
                   </div>
                   <div className="space-y-3">
                      <label className="text-[10px] text-slate-400 uppercase tracking-widest ml-6 leading-none">Plazo</label>
                      <select value={cuotas} onChange={e => setCuotas(e.target.value)} className="w-full bg-slate-50 dark:bg-white/[0.02] border border-slate-200 dark:border-white/10 rounded-[1.5rem] px-6 py-5 text-3xl text-slate-950 dark:text-white outline-none cursor-pointer">
                ...
                <div className="bg-indigo-600/5 p-8 rounded-[2.5rem] flex items-center border border-indigo-500/20 font-black italic">
                   ...
                   <div>
                     <p className="text-[9px] font-black text-indigo-500 uppercase tracking-widest leading-none mb-1">Amortización Bancaria</p>
                     <p className="text-xl uppercase italic tracking-tighter text-slate-900 dark:text-white">Sistema {sim?.sistema}</p>
                   </div>
                </div>
             </div>
             <div className="lg:col-span-5 bg-slate-950 p-16 flex flex-col justify-between text-white relative overflow-hidden font-black italic">
                ...
                <div className="text-center space-y-16 relative z-10">
                   ...
                   <div className="grid grid-cols-2 gap-6 font-black italic">
                      <div className="bg-white/5 p-6 rounded-[2rem] border border-white/10 backdrop-blur-xl">
                         <p className="text-[10px] text-slate-500 uppercase mb-2 opacity-60 leading-none">T.E.A.</p>
                         <p className="text-4xl tracking-tighter leading-none">{sim?.tea.toFixed(1)}%</p>
                      </div>
                      <div className="bg-white/5 p-6 rounded-[2rem] border border-white/10 backdrop-blur-xl">
                         <p className="text-[10px] text-emerald-500 uppercase mb-2 leading-none">C.F.T. Real</p>
                         <p className="text-4xl text-emerald-400 tracking-tighter leading-none">{sim?.cft.toFixed(1)}%</p>
                      </div>
                   </div>
                </div>
                <div className="pt-10 space-y-6 relative z-10 font-black italic uppercase">
                   ...
                   <button disabled={loading || !bcraData?.apto || (sim ? sim.cuota > cuadData?.margenAfectable : true)} className="w-full bg-indigo-600 text-white py-10 rounded-[2.5rem] font-black text-2xl shadow-xl hover:bg-indigo-500 hover:scale-[1.02] active:scale-95 transition-all tracking-widest uppercase italic leading-none">
                      Generar Activo
                   </button>
                </div>
             </div>
