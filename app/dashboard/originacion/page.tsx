...
export default function OriginacionPage() {
  ...
  // NUEVO: Ambos estados separados para CUAD y BCRA
  ...
  // NUEVO: Motor de consulta Dual (CUAD + BCRA)
  const handleConsultarDni = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    
    try {
      // Disparamos las DOS APIs al mismo tiempo (simuladas por ahora)
  ...
  const handleContinuar = async () => {
    ...
    try {
      const operacionRef = await addDoc(collection(db, 'operaciones'), {
        ...
        // Guardamos también el historial crediticio del momento de la consulta
        riesgoBcra: bcraData,
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto text-black bg-gray-50 min-h-screen">
      ...
      <div className="bg-white p-8 shadow-lg rounded-xl border border-gray-100 min-h-[400px]">
        ...
        {/* PASO 2: Simulación y Resultados Duales */}
        {step === 2 && cuadData && bcraData && (
          <div className="space-y-8 animate-fade-in-up">
            
            {/* NUEVO: Tarjetas de Información Duales */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4">
              ...
              <div className="bg-gray-800 text-white p-6 rounded-xl flex flex-col justify-center space-y-4 shadow-inner">
                ...
                {/* LÓGICA DE APROBACIÓN O RECHAZO DUAL */}
                {!bcraData.apto ? (
...
EOFcat << 'EOF' > app/dashboard/originacion/page.tsx
'use client';
