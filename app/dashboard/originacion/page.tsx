...
export default function OriginacionPage() {
  ...
  const calcularFinanzas = () => {
    ...
    const i = tna / 12;
    const capTotal = capital + (capital * ((p.gastosAdminPct || 0) / 100));
    ...
    <div className="space-y-10 animate-in fade-in duration-1000">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b dark:border-white/5 pb-8">
        <div className="space-y-1 font-black italic uppercase">
          <div className="flex items-center space-x-2 text-indigo-500 text-[10px] uppercase tracking-[0.5em] leading-none">
            <Cpu className="w-3 h-3 fill-current" /> <span>Smart Originator</span>
          </div>
          <h1 className="text-5xl tracking-tighter text-slate-950 dark:text-white leading-none">Simulación</h1>
        </div>
      </div>
      ...
      <div className="bg-white/70 dark:bg-[#0b1224]/70 backdrop-blur-3xl rounded-[3rem] shadow-2xl border border-slate-200/60 dark:border-white/5 overflow-hidden">
        {step === 1 && (
          <form onSubmit={handleConsultar} className="max-w-xl mx-auto py-24 px-10 space-y-12 animate-in zoom-in-95 font-black italic uppercase">
             <div className="space-y-4">
                <label className="text-[10px] text-slate-400 tracking-[0.4em] ml-6 leading-none italic">Canal Originador</label>
                <select value={selectedEntityId} onChange={e => setSelectedEntityId(e.target.value)} className="w-full bg-slate-50 dark:bg-white/[0.03] border border-slate-200 dark:border-white/10 rounded-[1.5rem] px-8 py-5 text-lg text-slate-900 dark:text-white outline-none focus:ring-4 focus:ring-indigo-500/10 transition-all appearance-none cursor-pointer">
                   {entities.map(e => <option key={e.id} value={e.id}>{e.fantasyName || e.name}</option>)}
                </select>
             </div>
             <div className="space-y-4 text-center">
                <label className="text-[10px] text-slate-400 tracking-[0.4em] leading-none italic">DNI Solicitante</label>
                <input type="text" required value={dni} onChange={e => setDni(e.target.value.replace(/\D/g, ''))} className="w-full text-center text-7xl p-4 bg-transparent border-b-[10px] border-indigo-600 outline-none text-slate-950 dark:text-white tracking-tighter leading-none" placeholder="00000000" />
             </div>
             <button type="submit" disabled={loading || dni.length < 7} className="w-full bg-indigo-600 text-white py-8 rounded-[2.5rem] text-xl shadow-2xl shadow-indigo-500/20 hover:scale-[1.02] active:scale-95 transition-all tracking-widest leading-none">
        ...
        {step === 2 && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-0 animate-in slide-in-from-bottom-10 font-black italic uppercase">
             <div className="lg:col-span-7 p-12 space-y-12 border-r dark:border-white/5">
                <div className="bg-slate-50/50 dark:bg-white/[0.02] p-8 rounded-[2.5rem] border border-slate-200 dark:border-white/5 shadow-inner flex items-center justify-between">
                   <div className="flex items-center space-x-6">
                      ...
                      <div>
                        <h3 className="text-2xl text-slate-950 dark:text-white leading-none tracking-tighter">{cuadData?.nombre}</h3>
                        <p className="text-xs text-indigo-500 tracking-widest mt-2 leading-none italic">BCRA SIT: {bcraData?.situacion}</p>
                      </div>
                   </div>
                   <div className="text-right font-black">
                      <p className="text-[10px] text-slate-400 tracking-widest leading-none mb-1">Margen</p>
                      <p className="text-xl text-slate-900 dark:text-white leading-none">${cuadData?.margenAfectable?.toLocaleString()}</p>
                   </div>
                </div>

                <div className="grid grid-cols-2 gap-8 font-black">
                   <div className="space-y-3">
                      <label className="text-[10px] text-slate-400 tracking-widest ml-6 leading-none">Monto ($)</label>
                   ...
                   <div className="space-y-3">
                      <label className="text-[10px] text-slate-400 tracking-widest ml-6 leading-none">Plazo</label>
                ...
                <div className="bg-indigo-600/5 p-8 rounded-[2.5rem] flex items-center border border-indigo-500/20">
                   ...
                   <div>
                     <p className="text-[9px] text-indigo-500 tracking-widest leading-none mb-1">Amortización Bancaria</p>
                     <p className="text-xl tracking-tighter text-slate-900 dark:text-white leading-none">Sistema {sim?.sistema}</p>
                   </div>
                </div>
             </div>

             <div className="lg:col-span-5 bg-slate-950 p-16 flex flex-col justify-between text-white relative overflow-hidden font-black italic uppercase">
                <div className="absolute inset-0 bg-gradient-to-br from-indigo-500/10 to-transparent"></div>
                <div className="text-center space-y-16 relative z-10 font-black">
                   <div className="space-y-2">
                      <p className="text-indigo-500 text-[11px] tracking-[0.5em] leading-none">Cuota Mensual Fija</p>
                      <h2 className="text-[8rem] tracking-tighter leading-none drop-shadow-[0_20px_40px_rgba(59,130,246,0.3)] leading-none">
                   ...
                   <div className="grid grid-cols-2 gap-6">
                      <div className="bg-white/5 p-6 rounded-[2rem] border border-white/10 backdrop-blur-xl">
                         <p className="text-[10px] text-slate-500 tracking-widest mb-2 opacity-60 leading-none">T.E.A.</p>
                         <p className="text-4xl tracking-tighter leading-none">{sim?.tea.toFixed(1)}%</p>
                      </div>
                      <div className="bg-white/5 p-6 rounded-[2rem] border border-white/10 backdrop-blur-xl">
                         <p className="text-[10px] text-emerald-500 tracking-widest mb-2 leading-none">C.F.T. Real</p>
                         <p className="text-4xl text-emerald-400 tracking-tighter leading-none">{sim?.cft.toFixed(1)}%</p>
                      </div>
                   </div>
                </div>
                
                <div className="pt-10 space-y-6 relative z-10 font-black">
                   <div className={`p-6 rounded-[2rem] text-center text-[10px] tracking-[0.3em] ${bcraData?.apto && sim?.cuota <= cuadData?.margenAfectable ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400' : 'bg-rose-500/10 border-rose-500/30 text-rose-400'}`}>
                      {bcraData?.apto && sim?.cuota <= cuadData?.margenAfectable ? '✓ APTO ORIGINACIÓN' : '✕ RECHAZO AUTOMÁTICO'}
                   </div>
                   <button disabled={loading || !bcraData?.apto || (sim ? sim.cuota > cuadData?.margenAfectable : true)} className="w-full bg-indigo-600 text-white py-10 rounded-[2.5rem] text-2xl shadow-xl hover:bg-indigo-500 hover:scale-[1.02] active:scale-95 transition-all tracking-widest leading-none">
                      Generar Activo
                   </button>
                </div>
             </div>
