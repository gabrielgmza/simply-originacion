  ...
  return (
    <div className="max-w-4xl mx-auto space-y-10 animate-in fade-in duration-1000 italic font-bold uppercase leading-none">
      <header className="border-b dark:border-white/5 pb-6 leading-none italic font-bold uppercase">
        <h1 className="text-4xl font-black tracking-tighter text-slate-900 dark:text-white italic uppercase leading-none">Mi Perfil</h1>
        <p className="text-[11px] font-bold text-slate-400 tracking-[0.4em] uppercase mt-2 italic">Ajustes del Operador Master Simply</p>
      </header>

      <div className="flex items-center space-x-8 bg-white dark:bg-[#111111] p-10 rounded-[3rem] shadow-xl border border-slate-100 dark:border-white/5 leading-none italic font-bold uppercase group">
         <div className="w-24 h-24 bg-gradient-to-tr from-[#FF5E14] to-orange-400 rounded-3xl shadow-2xl flex items-center justify-center text-white font-black text-4xl transform -rotate-2 group-hover:rotate-0 transition-transform duration-500 italic">
         ...
         <div className="italic font-bold uppercase leading-none">
            ...
            <div className="mt-4 flex space-x-2">
               <span className="bg-emerald-500/10 text-emerald-500 px-3 py-1.5 rounded-lg text-[9px] tracking-widest font-black uppercase italic border border-emerald-500/20">STATUS: CONECTADO</span>
            </div>
         </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 leading-none italic font-bold uppercase">
        <Option icon={Shield} title="Seguridad" desc="Autenticación Biométrica y 2FA" />
        <Option icon={Bell} title="Notificaciones" desc="Push de Originación y BCRA" />
        <Option icon={Key} title="Llaves API" desc="Gestión de acceso programático" />
        <Option icon={Globe} title="Regional" desc="Zona Horaria Mendoza, ARG" />
      </div>
    </div>
  );
}
