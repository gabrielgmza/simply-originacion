  ...
  return (
    <div className="max-w-4xl mx-auto space-y-10 animate-in fade-in duration-1000 italic font-bold uppercase leading-none">
      <header className="border-b dark:border-white/5 pb-6 leading-none italic font-bold uppercase">
        <h1 className="text-4xl font-black tracking-tighter text-slate-900 dark:text-white italic uppercase leading-none">Perfil</h1>
      ...
      <div className="flex items-center space-x-8 bg-white dark:bg-[#111111] p-10 rounded-[3rem] shadow-xl border border-slate-100 dark:border-white/5 leading-none italic font-bold uppercase group transition-all duration-500 hover:shadow-orange-600/5">
         ...
         <div className="italic font-bold uppercase leading-none text-slate-900 dark:text-white">
            <h2 className="text-2xl font-black tracking-tighter leading-none italic font-black uppercase">{user?.email?.split('@')[0]}</h2>
            ...
            <div className="mt-4 flex space-x-2">
               <span className="bg-emerald-500/10 text-emerald-500 px-3 py-1.5 rounded-lg text-[9px] tracking-widest font-black uppercase italic border border-emerald-500/20 shadow-sm leading-none">STATUS: CONECTADO</span>
            </div>
         </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 leading-none italic font-bold uppercase">
        <Option icon={Shield} title="Seguridad" desc="Autenticación 2FA" />
        <Option icon={Bell} title="Alertas" desc="Push Originación" />
        <Option icon={Key} title="Llaves" desc="Acceso API Fondeo" />
        <Option icon={Globe} title="Regional" desc="Zona Mendoza, AR" />
      </div>
    </div>
  );
}
