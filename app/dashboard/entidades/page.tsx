...
export default function EntidadesPage() {
  ...
  const Input = ({ label, icon: Icon, value, onChange, type = "text" }: any) => (
    <div className="space-y-2.5 text-slate-900 dark:text-white font-black italic">
  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-1000">
      {toast && (
        <div className="fixed bottom-10 right-10 z-[100] bg-indigo-600 text-white px-8 py-4 rounded-[2rem] shadow-2xl font-black text-[10px] uppercase tracking-widest animate-in slide-in-from-right-10 leading-none shadow-indigo-500/30">
          {toast}
        </div>
      )}
      ...
      <div className="flex items-end justify-between border-b dark:border-white/5 pb-10">
        <div className="space-y-2">
          ...
          <p className="text-xs font-bold text-slate-400 uppercase tracking-[0.4em]">Node Administration SaaS</p>
        </div>
        <button className="bg-indigo-600 text-white px-8 py-3.5 rounded-[1.5rem] font-black text-[10px] uppercase tracking-widest shadow-2xl hover:scale-105 transition-all italic leading-none shadow-indigo-500/20">
           <Plus className="w-4 h-4 mr-2" /> Adherir Nodo
        </button>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 text-slate-950 dark:text-white font-black italic">
            ...
            <form onSubmit={handleSubmit} className="space-y-14">
              <div className="space-y-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                  ...
                  <div className="bg-indigo-600 p-8 rounded-[2.2rem] flex flex-col justify-center text-center shadow-2xl ring-8 ring-indigo-50 dark:ring-indigo-500/10 group hover:scale-[1.02] transition-transform shadow-indigo-500/30">
              ...
              <div className="flex justify-end pt-6 font-black italic">
                <button type="submit" disabled={loading} className="bg-slate-950 dark:bg-indigo-600 text-white px-14 py-5 rounded-[2rem] font-black uppercase tracking-[0.2em] text-[11px] shadow-2xl hover:scale-105 active:scale-95 transition-all flex items-center italic leading-none shadow-indigo-500/20">
        ...
        <div className="space-y-10">
          ...
          <div className="space-y-4">
            <h5 className="text-[10px] font-black uppercase tracking-[0.5em] text-slate-400 ml-8 leading-none italic">Nodos Activos</h5>
