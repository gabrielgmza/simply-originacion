...
// Ruta relativa manual para evitar errores de alias
...
export default function EntidadesPage() {
  ...
  const Input = ({ label, icon: Icon, value, onChange, type = "text" }: any) => (
    <div className="space-y-2.5">
      <label className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-4 leading-none flex items-center">
    ...
    <div className="space-y-12 animate-in fade-in duration-1000">
      ...
      <div className="flex items-end justify-between border-b dark:border-white/5 pb-10">
        <div className="space-y-2">
          ...
          <p className="text-xs font-bold text-slate-400 uppercase tracking-[0.4em]">Core Administration Node</p>
        </div>
        <button className="bg-indigo-600 text-white px-8 py-4 rounded-[1.5rem] font-black text-[10px] uppercase tracking-widest shadow-2xl hover:scale-105 active:scale-95 transition-all italic flex items-center leading-none">
           <Plus className="w-4 h-4 mr-2" /> Adherir Nodo
        </button>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 text-slate-900 dark:text-white font-bold">
          ...
          <div className="bg-white/80 dark:bg-[#0b1224]/80 backdrop-blur-xl p-12 rounded-[3rem] shadow-xl border border-slate-200/60 dark:border-white/5">
            <form onSubmit={handleSubmit} className="space-y-14">
              <div className="space-y-10">
                <h3 className="text-xs font-black uppercase tracking-[0.3em] text-indigo-500 border-b dark:border-white/5 pb-4 leading-none italic">Contratación y Registro</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                  ...
                  <div className="bg-indigo-600 p-8 rounded-[2.5rem] flex flex-col justify-center text-center shadow-2xl ring-8 ring-indigo-50 dark:ring-indigo-500/10 group hover:scale-[1.02] transition-transform">
              ...
              <div className="space-y-10 pt-10 border-t dark:border-white/5">
                <h3 className="text-xs font-black uppercase tracking-[0.3em] text-emerald-500 border-b dark:border-white/5 pb-4 leading-none italic">Parámetros Bancarios</h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
                  <div className="space-y-3">
                    <label className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-5 leading-none flex items-center italic">
              ...
              <div className="flex justify-end pt-6">
                <button type="submit" disabled={loading} className="bg-slate-950 dark:bg-indigo-600 text-white px-14 py-5 rounded-[2rem] font-black uppercase tracking-[0.2em] text-[11px] shadow-2xl hover:scale-105 active:scale-95 transition-all flex items-center italic leading-none">
        ...
        <div className="space-y-10">
          <div className="bg-slate-950 p-10 rounded-[3rem] text-white shadow-2xl relative overflow-hidden border border-white/5 ring-1 ring-white/10 group">
             ...
             <p className="text-sm font-medium text-slate-400 leading-relaxed relative z-10 opacity-90 leading-relaxed italic">
               La configuración del Core impacta en tiempo real. Los cambios en el sistema de amortización solo afectarán a las nuevas simulaciones generadas desde este nodo centralizado.
             </p>
          </div>
          ...
          <div className="space-y-5">
            ...
            <div className="space-y-4">
              {entities.map(ent => (
                <div key={ent.id} onClick={() => { setIsEditing(true); setCurrentId(ent.id); }} className="bg-white/40 dark:bg-[#0b1224]/40 backdrop-blur-md p-7 rounded-[2.5rem] border border-slate-200 dark:border-white/5 hover:border-indigo-500 transition-all cursor-pointer group flex justify-between items-center shadow-sm">
                  <div>
                    ...
                    <p className="text-[10px] font-bold text-indigo-500 mt-1 uppercase tracking-widest leading-none">SaaS Fee: {ent.comisionSaaS}%</p>
                  </div>
