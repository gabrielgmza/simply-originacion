...
import { Building2, ShieldCheck, Zap, ArrowRight, Save, Plus, Percent, CreditCard } from 'lucide-react';
...
export default function EntidadesPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditing && currentId) {
        ...
        showToast("Sistema Sincronizado");
      } else {
        await addDoc(collection(db, 'entities'), payload);
        showToast("Entidad Registrada");
      }
      setIsEditing(false);
      fetchEntities();
  ...
  const Input = ({ label, icon: Icon, value, onChange, type = "text" }: any) => (
    <div className="space-y-2.5 text-slate-900 dark:text-white font-black italic uppercase">
      <label className="text-[10px] text-slate-400 dark:text-slate-500 tracking-widest ml-5 flex items-center leading-none italic">
        <Icon className="w-3.5 h-3.5 mr-2 text-indigo-500" /> {label}
      </label>
      <input type={type} value={value} onChange={onChange} className="w-full bg-slate-50 dark:bg-white/[0.02] border border-slate-200 dark:border-white/10 rounded-2xl px-6 py-4 text-[13px] font-bold outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all shadow-sm" />
    </div>
  );
    ...
    <div className="space-y-12 animate-in fade-in duration-1000">
      ...
      <div className="flex items-end justify-between border-b dark:border-white/5 pb-10">
        <div className="space-y-2">
          ...
          <p className="text-xs font-bold text-slate-400 uppercase tracking-[0.4em]">Node Management Core</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 text-slate-900 dark:text-white font-black uppercase italic">
        <div className="lg:col-span-2">
          <Card className="p-12">
            <form onSubmit={handleSubmit} className="space-y-14">
              <div className="space-y-10">
                <h3 className="text-xs font-black uppercase tracking-[0.3em] text-indigo-500 border-b dark:border-white/5 pb-4 leading-none italic">Identidad Corporativa</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                  ...
                  <div className="bg-indigo-600 p-8 rounded-[2.2rem] flex flex-col justify-center text-center shadow-2xl shadow-indigo-500/30">
                     <label className="text-[10px] font-black text-indigo-200 uppercase tracking-widest mb-1 opacity-70 leading-none italic">SaaS Fee (%)</label>
              ...
              <div className="space-y-10 pt-10 border-t dark:border-white/5">
                <h3 className="text-xs font-black uppercase tracking-[0.3em] text-emerald-500 border-b dark:border-white/5 pb-4 leading-none italic">Motor Bancario</h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                  <div className="space-y-3 font-black italic">
                    <label className="text-[10px] text-slate-400 dark:text-slate-500 tracking-widest ml-5 leading-none flex items-center italic">
                       <CreditCard className="w-3.5 h-3.5 mr-2 text-emerald-500" /> Sistema
                    </label>
                    <select value={financialData.sistemaAmortizacion} onChange={(e:any) => setFinancialData({...financialData, sistemaAmortizacion: e.target.value})} className="w-full bg-slate-50 dark:bg-white/[0.02] border border-slate-200 dark:border-white/10 rounded-2xl px-6 py-4 text-[13px] font-bold outline-none focus:border-emerald-500 appearance-none cursor-pointer text-slate-900 dark:text-white italic leading-none">
                  ...
                  <Input icon={Zap} label="Gastos Otorg. (%)" type="number" value={financialData.gastosOtorgamientoPct} onChange={(e:any) => setFinancialData({...financialData, gastosOtorgamientoPct: parseFloat(e.target.value)})} />
                </div>
              </div>

              <div className="flex justify-end pt-6 italic">
                <button type="submit" disabled={loading} className="bg-slate-950 dark:bg-indigo-600 text-white px-14 py-5 rounded-[2rem] font-black uppercase tracking-[0.2em] text-[11px] shadow-2xl hover:scale-105 active:scale-95 transition-all flex items-center shadow-indigo-500/20 italic leading-none">
                  <Save className="w-4 h-4 mr-3" /> {isEditing ? 'Sincronizar' : 'Activar Entidad'}
                </button>
              </div>
            </form>
          </Card>
        ...
        <div className="space-y-10">
          <Card className="p-10 bg-slate-950 text-white border-none relative overflow-hidden">
             <div className="absolute inset-0 bg-indigo-500/5 transition-colors duration-500"></div>
             <ShieldCheck className="w-12 h-12 mb-8 text-indigo-500 relative z-10 opacity-60" />
             <h4 className="text-2xl font-black italic uppercase tracking-tighter mb-6 relative z-10 leading-none italic">Security Architecture</h4>
          ...
          <div className="space-y-5">
            <h5 className="text-[10px] font-black uppercase tracking-[0.5em] text-slate-400 ml-8 leading-none italic">Operadoras Activas</h5>
            {entities.map(ent => (
              <div key={ent.id} onClick={() => { setIsEditing(true); setCurrentId(ent.id); }} className="bg-white/40 dark:bg-[#0b1224]/40 backdrop-blur-md p-7 rounded-[2.5rem] border border-slate-200/60 dark:border-white/5 hover:border-indigo-500 transition-all cursor-pointer group flex justify-between items-center shadow-sm">
                <div>
                  ...
                  <p className="text-[10px] font-bold text-indigo-500 mt-1 uppercase tracking-widest leading-none font-black italic opacity-60 leading-none">SaaS: {ent.comisionSaaS}%</p>
                </div>
