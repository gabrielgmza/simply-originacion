...
import { db } from '../lib/firebase-bridge';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditing && currentId) {
        ...
        setToast("Nodo Activo");
      }
      setIsEditing(false);
      fetchEntities();
    } catch (e) { setToast("Error de red"); }
    finally { setLoading(false); }
  };
    ...
    <div className="space-y-12 animate-in fade-in duration-1000">
      ...
      <div className="flex items-end justify-between border-b dark:border-white/5 pb-10">
        <div className="space-y-2">
          ...
          <p className="text-xs font-bold text-slate-400 uppercase tracking-[0.4em]">Core Admin SaaS</p>
        </div>
        <button className="bg-indigo-600 text-white px-8 py-3.5 rounded-[1.5rem] font-black text-[10px] uppercase tracking-widest shadow-2xl hover:scale-105 transition-all italic">
           <Plus className="w-4 h-4 mr-2" /> Adherir Nodo
        </button>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 text-slate-950 dark:text-white font-black">
            ...
            <form onSubmit={handleSubmit} className="space-y-14">
              <div className="space-y-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                  ...
                  <div className="bg-indigo-600 p-8 rounded-[2.2rem] flex flex-col justify-center text-center shadow-2xl ring-8 ring-indigo-50 dark:ring-indigo-500/5 group hover:scale-[1.02] transition-transform">
                     <label className="text-[10px] font-black text-indigo-200 uppercase tracking-widest mb-1 opacity-70 italic">SaaS Fee (%)</label>
