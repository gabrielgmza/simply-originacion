...
export default function EntidadesPage() {
  ...
  const Input = ({ label, icon: Icon, value, onChange, type = "text" }: any) => (
    <div className="space-y-2.5 text-slate-900 dark:text-white">
      ...
      <input type={type} value={value} onChange={onChange} className="w-full bg-slate-50 dark:bg-white/[0.03] border border-slate-200 dark:border-white/10 rounded-2xl px-6 py-4 text-[13px] font-bold outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all shadow-sm" />
    </div>
  );
  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-1000">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 border-b dark:border-white/5 pb-10">
        ...
        <button className="bg-indigo-600 text-white px-8 py-3.5 rounded-[2rem] font-black text-[10px] uppercase tracking-widest shadow-2xl shadow-indigo-500/20 hover:scale-105 active:scale-95 transition-all flex items-center italic">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        <div className="lg:col-span-2">
          <div className="bg-white dark:bg-[#0b1224] p-10 rounded-[3rem] shadow-xl border border-slate-200 dark:border-white/5">
            <form onSubmit={handleSubmit} className="space-y-12">
              <div className="space-y-8">
                <h3 className="text-xs font-black uppercase tracking-widest text-indigo-500 border-b dark:border-white/5 pb-4">Identidad Corporativa</h3>
              ...
              <div className="space-y-8 pt-10 border-t dark:border-white/5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                  ...
                  <Input icon={Percent} label="Gastos Otorg. (%)" type="number" value={financialData.gastosOtorgamientoPct} onChange={(e:any) => setFinancialData({...financialData, gastosOtorgamientoPct: parseFloat(e.target.value)})} />
                </div>
              </div>

              <div className="flex justify-end pt-6">
                <button type="submit" disabled={loading} className="bg-slate-950 dark:bg-indigo-600 text-white px-12 py-5 rounded-[2rem] font-black uppercase tracking-[0.2em] text-[11px] shadow-2xl hover:scale-105 active:scale-95 transition-all flex items-center italic">
        ...
        <div className="space-y-8">
          <div className="bg-slate-950 p-10 rounded-[3rem] text-white shadow-2xl relative overflow-hidden border border-white/5 ring-1 ring-white/10 group">
             ...
             <h4 className="text-2xl font-black italic uppercase tracking-tighter mb-4 relative z-10 leading-none">Security Architecture</h4>
             <p className="text-sm font-medium text-slate-400 leading-relaxed relative z-10 opacity-90 leading-relaxed">
               La configuración del Core impacta en tiempo real. Los cambios en el sistema de amortización solo afectarán a las nuevas solicitudes de originación.
             </p>
          </div>

          <div className="space-y-4 font-black">
            <h5 className="text-[10px] uppercase tracking-[0.5em] text-slate-400 ml-8 leading-none">Red de Operadoras</h5>
            <div className="space-y-4">
              {entities.map(ent => (
                <div key={ent.id} onClick={() => { setIsEditing(true); setCurrentId(ent.id); }} className="bg-white/40 dark:bg-[#0b1224]/40 backdrop-blur-md p-7 rounded-[2.5rem] border border-slate-200/60 dark:border-white/5 hover:border-indigo-500 transition-all cursor-pointer group shadow-sm flex justify-between items-center">
                  <div>
                    <p className="text-sm uppercase italic tracking-tighter text-slate-950 dark:text-white leading-none mb-2">{ent.name}</p>
