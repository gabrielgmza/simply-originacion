...
// Ruta relativa manual para evitar errores de alias en Vercel
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className={isDark ? 'dark' : ''}>
      <div className="flex h-screen bg-[#f8fafc] dark:bg-[#020617] text-slate-900 dark:text-slate-100 transition-colors duration-700 font-sans">
        ...
        <aside className="w-72 bg-white/80 dark:bg-[#0b1224]/80 backdrop-blur-2xl border-r border-slate-200/60 dark:border-white/5 flex flex-col z-50">
          <div className="p-10 flex items-center space-x-3 group">
            <div className="w-10 h-10 bg-indigo-600 rounded-2xl flex items-center justify-center text-white shadow-2xl shadow-indigo-500/20 group-hover:rotate-12 transition-all duration-500">
              <span className="text-xl font-black italic tracking-tighter">S</span>
            </div>
            <div className="leading-none">
              <h1 className="text-lg font-black tracking-tighter italic uppercase">Simply</h1>
              <span className="text-[9px] uppercase tracking-[0.4em] font-bold text-indigo-500/80">Fintech Core</span>
            </div>
          </div>
              ...
              return (
                ...
                >
                  <div className="flex items-center">
                    ...
                    <span className={`text-[13px] tracking-tight ${active ? 'font-bold' : 'font-semibold italic'}`}>{item.name}</span>
                  </div>
          ...
          <div className="p-8 space-y-6">
            ...
            <button onClick={() => signOut(auth)} className="w-full text-center py-2 text-[10px] font-black text-rose-500 uppercase tracking-widest opacity-60 hover:opacity-100 transition-opacity flex items-center justify-center italic">
              <LogOut className="w-3.5 h-3.5 mr-2" /> Salir del Sistema
            </button>
          </div>
        </aside>
        ...
        <main className="flex-1 flex flex-col relative overflow-hidden">
          <header className="h-24 flex items-center justify-between px-12 bg-white/40 dark:bg-[#020617]/40 backdrop-blur-md z-40 border-b border-slate-100 dark:border-white/5">
            <div className="flex items-center bg-slate-100 dark:bg-white/[0.03] px-6 py-3 rounded-[1.2rem] w-96 border border-transparent focus-within:ring-4 focus-within:ring-indigo-500/10 transition-all">
              ...
              <input placeholder="Buscar activos..." className="bg-transparent border-none outline-none text-xs w-full font-semibold placeholder:text-slate-400" />
            </div>
