...
import { auth } from '../lib/firebase-bridge';
import { 
      ...
      <div className="flex h-screen bg-[#fcfdfe] dark:bg-[#020617] text-slate-900 dark:text-slate-100 transition-colors duration-700 font-sans selection:bg-indigo-500/30">
        ...
        <aside className="w-72 m-4 mr-0 bg-white/70 dark:bg-[#0b1224]/70 backdrop-blur-2xl border border-slate-200/60 dark:border-white/5 flex flex-col z-50 rounded-[2.5rem] shadow-2xl shadow-indigo-500/5">
          <div className="p-8 flex items-center space-x-3 group">
            <div className="w-10 h-10 bg-indigo-600 rounded-2xl flex items-center justify-center text-white shadow-xl shadow-indigo-500/20 group-hover:rotate-12 transition-all duration-500 font-black italic">S</div>
            <div>
              ...
              return (
                ...
                >
                  <div className="flex items-center italic">
                    ...
                    <span className={`text-[13px] tracking-tight font-black`}>{item.name}</span>
                  </div>
          ...
          <div className="p-6 mt-auto space-y-4">
            ...
            <button onClick={() => signOut(auth)} className="w-full flex items-center px-5 py-3 text-[10px] font-black text-rose-500 uppercase tracking-widest hover:opacity-100 opacity-60 transition-all">
              <LogOut className="w-4 h-4 mr-3" /> Salir
            </button>
          </div>
        </aside>
