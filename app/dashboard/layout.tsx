...
import { LayoutDashboard, UserCheck, Briefcase, Users, FileSignature, Webhook, Palette, Menu, X, LogOut, BadgeCheck, Key, TrendingUp, Banknote, PhoneCall, MessageSquare, Bell, Calculator, Building2, CreditCard } from "lucide-react";
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const menu = [
    ...
    { nombre: "Credito CUAD",          ruta: "/dashboard/cuad",                       visible: puedeVender },
    ...
    { nombre: "Leads",                 ruta: "/dashboard/leads",                      visible: puedeVender },
    { nombre: "Renovaciones",          ruta: "/dashboard/renovaciones",               visible: puedeVender },
    { nombre: "Pagos 360",             ruta: "/dashboard/pagos360",                   visible: puedeLiquidar },
    { nombre: "Liquidacion Masiva",    ruta: "/dashboard/liquidacion/masiva",         visible: puedeLiquidar },
    { nombre: "Revision Onboarding",   ruta: "/dashboard/revision",                   visible: puedeAprobar },
    ...
    { nombre: "Politica de Mora",      ruta: "/dashboard/configuracion/mora",         visible: puedeConfigurar },
    ...
    { nombre: "Liquidacion Config",    ruta: "/dashboard/configuracion/liquidacion",  visible: puedeConfigurar },
    { nombre: "Fondeadores",           ruta: "/dashboard/configuracion/fondeadores",  visible: puedeConfigurar },
    { nombre: "Email",                 ruta: "/dashboard/configuracion/email",        visible: puedeConfigurar },
  ];
  const iconos: Record<string, React.ReactNode> = {
    "/dashboard":                              <LayoutDashboard size={20} />,
    "/dashboard/originador/legajo":            <UserCheck size={20} />,
    "/dashboard/cuad":                         <CreditCard size={20} />,
    ...
    <div className="min-h-screen bg-black flex flex-col md:flex-row font-sans">
      <div className="md:hidden flex items-center justify-between bg-[#0A0A0A] border-b border-gray-800 p-4 sticky top-0 z-50">
        <div className="flex items-center gap-2">
          {logoUrl ? <img src={logoUrl} alt={nombreEntidad} className="h-8 object-contain" /> : <>
      ...
      <aside className={`${menuAbierto ? "translate-x-0" : "-translate-x-full"} md:translate-x-0 fixed md:static inset-y-0 left-0 z-40 w-64 bg-[#0A0A0A] border-r border-gray-800 flex flex-col transition-transform duration-300 ease-in-out`}>
        <div className="hidden md:flex items-center gap-2 p-6">
          {logoUrl ? <img src={logoUrl} alt={nombreEntidad} className="h-9 object-contain" /> : <>
          ...
          {menu.filter(item => item.visible).map((item) => {
            ...
            return (
              <Link key={item.ruta} href={item.ruta} onClick={() => setMenuAbierto(false)}
                className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-bold text-sm ${activo ? "text-white" : "text-gray-400 hover:text-white hover:bg-gray-900"}`}
                style={activo ? { backgroundColor: `${colorPrimario}22`, border: "1px solid", borderColor: `${colorPrimario}44` } : {}}>
                <span style={activo ? { color: colorPrimario } : {}}>{iconos[item.ruta] || <LayoutDashboard size={20} />}</span>
                {item.nombre}
              </Link>
            );
          })}
        ...
        <div className="p-4 border-t border-gray-800 space-y-2">
          ...
          <button onClick={cerrarSesion} className="flex items-center gap-3 px-4 py-3 text-red-500 hover:bg-red-950/30 rounded-xl transition-all font-bold text-sm w-full">
