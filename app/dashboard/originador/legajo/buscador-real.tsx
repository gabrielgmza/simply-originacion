...
import { Search, Gavel, Landmark, Briefcase, Loader2, AlertTriangle, X, RefreshCw, AlertCircle, FileText, Download, Info } from "lucide-react";
...
export default function BuscadorScoringReal({ alBuscarExitosamente }: Props) {
  ...
  // BCRA via API interna con caché
  ...
  // Juicios va directo al bot (no necesita credenciales ni caché)
  const consultarJuicios = async () => {
    setStatusJuicios("procesando");
    try {
      const res = await fetch(`${BOT_URL}/api/consultar-juicios`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
  ...
  // CUAD via API interna (gestiona credenciales de la entidad)
  const consultarCupo = async () => {
    ...
    try {
      ...
      if (data.gobiernoNoDisponible) setStatusBot("gobierno_caido");
      else if (data.error) setStatusBot("error");
                      ...
                      <div key={i} className="bg-black p-4 rounded-xl border border-gray-800">
                        ...
                        {parseInt(d.situacion) > 1 && (
                          <div className="mt-3 pt-3 border-t border-gray-800 flex justify-between items-center">
                            ...
                            <span className="text-orange-500 text-xs font-bold border border-orange-900 bg-orange-900/20 px-2 py-1 rounded">+15% Mora</span>
                          </div>
                        )}
                      </div>
                    ))}
