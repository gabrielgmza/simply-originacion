...
export default function DashboardPage() {
  ...
  const Card = ({ title, value, icon: Icon, color }: any) => (
    <div className="bg-white dark:bg-[#111111] p-8 rounded-[2.5rem] shadow-xl border border-slate-100 dark:border-white/5 relative overflow-hidden group">
      <div className={`absolute top-0 right-0 w-32 h-32 bg-${color === 'orange' ? '[#FF5E14]' : color}-500/10 blur-3xl -mr-16 -mt-16 group-hover:bg-orange-500/20 transition-all duration-500`}></div>
      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b dark:border-white/5 pb-6">
        <div className="space-y-1 font-black leading-none">
          <div className="flex items-center space-x-2 text-[#FF5E14] text-[10px] tracking-[0.5em] italic">
            <Activity className="w-4 h-4" /> <span>Métricas en Tiempo Real</span>
          </div>
          <h1 className="text-4xl tracking-tighter text-slate-900 dark:text-white leading-none italic">Visión General</h1>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-8 bg-white dark:bg-[#111111] p-10 rounded-[3rem] shadow-xl border border-slate-100 dark:border-white/5 font-black italic">
          <div className="flex items-center justify-between mb-10">
             <h3 className="text-xl text-slate-900 dark:text-white tracking-tighter uppercase italic font-black">Actividad Reciente</h3>
             <button className="text-[10px] text-[#FF5E14] tracking-widest font-black flex items-center hover:translate-x-1 transition-transform">VER REGISTRO <ArrowRight className="w-4 h-4 ml-2" /></button>
          </div>
          <div className="space-y-6 opacity-30 pointer-events-none italic">
             {[1,2,3].map(i => (
               <div key={i} className="h-16 bg-slate-100 dark:bg-white/5 rounded-2xl animate-pulse flex items-center px-6">
        ...
        <div className="lg:col-span-4 bg-[#FF5E14] p-10 rounded-[3rem] text-white shadow-[0_40px_80px_-15px_rgba(255,94,20,0.4)] relative overflow-hidden flex flex-col justify-between italic font-black group">
           ...
           <div>
              <h4 className="text-3xl tracking-tighter leading-tight mb-4 italic font-black">Nueva Operación</h4>
              <p className="text-orange-50 text-xs font-medium leading-relaxed opacity-80 mb-10 uppercase italic font-bold">Inicia el proceso de scoring y liquidación masiva en segundos.</p>
              <button className="w-full bg-white text-[#FF5E14] py-5 rounded-2xl text-[11px] tracking-widest hover:bg-orange-50 transition-all uppercase italic font-black shadow-xl">Simular Ahora</button>
           </div>
        </div>
      </div>
    </div>
